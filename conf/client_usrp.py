...
def receive_image_with_header(n,ip):
    try:
        client_ip = f"10.45.0.{ip}"
        client_port = 9000
...
if __name__ == '__main__':
    ...
    print(f"P: {args.ip}, N: {args.number},")
    
    # 4개의 프로세스 생성
    processes = []
    for i in range(args.number):
        p = mp.Process(target=receive_image_with_header, args=(i,args.ip))
        p.start()
        processes.append(p)
